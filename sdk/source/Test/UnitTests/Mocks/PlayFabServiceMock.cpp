...
rapidjson::MemoryPoolAllocator<>& PlayFabServiceMock::JsonAllocator() noexcept
{
    return s_serviceResponses.GetAllocator();
}

void PlayFabServiceMock::HttpMockCallback(HttpMock const& mock, std::string url, std::string requestBody) noexcept
{
    std::unique_lock<std::mutex> lock{ m_mutex };
    if (m_callback)
    {
        m_callback(*this, std::move(url), std::move(requestBody));
    }
    HttpMock::SetResponseBody(m_responseBody);
}

std::string PlayFabServiceMock::GetUrl(const char* connectionString, const char* apiName) noexcept
{
