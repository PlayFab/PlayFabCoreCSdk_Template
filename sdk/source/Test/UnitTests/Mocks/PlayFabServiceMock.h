...
{
public:
    using Callback = std::function<void(PlayFabServiceMock& mock, std::string url, std::string requestBody)>;
    ...
    // "data" field of a successful service responses. Will cause an assertion if the field isn't presenst (ex. the response
    ...
    static rapidjson::MemoryPoolAllocator<>& JsonAllocator() noexcept;

private:
    void HttpMockCallback(HttpMock const& mock, std::string url, std::string requestBody) noexcept;

    static std::string GetUrl(const char* connectionString, const char* apiName) noexcept;
...
PlayFabServiceMock::PlayFabServiceMock(Wrappers::ServiceConfig<Alloc> const& serviceConfig, const char* apiName) :
    HttpMock{ "POST", GetUrl(serviceConfig.ConnectionString().data(), apiName).data() },
    m_responses{ GetServiceResponses(apiName) }
{
    ...
    HttpMock::SetCallback(std::bind(&PlayFabServiceMock::HttpMockCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
}
