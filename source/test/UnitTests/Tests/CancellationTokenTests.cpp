...
public:
    ...
    TEST_METHOD(CancelMultipleOperationsWithParentToken)
    {
        ...
        AwaitCancelAsync(derivedToken).Finally(continuation);
        AwaitCancelAsync(derivedToken.Derive()).Finally(continuation);
        AwaitCancelAsync(derivedToken.Derive().Derive()).Finally(continuation);
